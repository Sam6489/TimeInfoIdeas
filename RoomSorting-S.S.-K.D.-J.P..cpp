...
void AllData:: fill( )
{
	...
	getcourseinfo.open( "MasterFile.csv" );
	...
	cout << "About to create array" << endl;
	...
	cout << "Getting room info" << endl;

	ifstream getRoomInfo;
	getRoomInfo.open( "Ad Astra LC Room Description List.csv" );

	string rawRoomData[MAX];
	string roomInformation[16];

	roomInfoSize = 0;
	getline( getRoomInfo, rawRoomData[0] );
	while ( !getRoomInfo.fail( ) && roomInfoSize < MAX )
	{
		roomInfoSize++;
		getline( getRoomInfo, rawRoomData[roomInfoSize] );
	}
	parser.parseAll( roomInformation, rawRoomData[1] );
	myRooms = new RoomInfo[roomInfoSize];
	myRooms[0].fillAll( roomInformation[6] );
	for ( int j = 0; j < roomInfoSize - 1; j++ )
	{
		parser.parseAll( roomInformation, rawRoomData[j + 1] );
		myRooms[j].fillAll( roomInformation[6] );
		cout << roomInformation[6] << endl;
	}
}
...
void CourseInfo::checkForLink( CourseInfo toCompare )
{
	...
	//Compares two Courses to find scheduling conflicts
...
string ParseInfo::convertToMiltary( string time )
{
	// -------------Checks--------------
